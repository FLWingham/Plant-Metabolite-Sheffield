...
def remove_rootdir_path(s):
    badSubstrings = [rootdir]
    for badSubstring in badSubstrings:
        new_s = s.replace(badSubstring, "")
    return new_s

#===================================================================================================================================#

def pca_output_name(filename):
    filename_no_root = remove_rootdir_path(filename)
    badSubstrings = ["\\"]
    for badSubstring in badSubstrings:
        new_filename = filename_no_root.replace(badSubstring, "_")
    return new_filename
...
def print_current_dir(directory_path,CLI_output_type):
    ...
    if CLI_output_type == 'initial_PCA':

        d_dirs_initial = [n for n in d_dirs if 'pca_results' not in n and 'raw' not in n] 
        pretty_print(user_identifier(d_dirs_initial))
        return d_dirs_initial

    if CLI_output_type == 'initial_filter':

        d_dirs_initial = [n for n in d_dirs if 'pca_results' not in n and 'raw' in n] 
        pretty_print(user_identifier(d_dirs_initial))
        return d_dirs_initial 


    elif CLI_output_type == 'PCA':

        if d_dirs:

            d_dirs_PCA = [n for n in d_dirs if not 'raw' in n]
            pretty_print(user_identifier(d_dirs_PCA))
            return d_dirs_PCA
    ...
    elif CLI_output_type == 'filter':

        if d_dirs:

            d_dirs_filter = [n for n in d_dirs if 'raw' in n]
            pretty_print(user_identifier(d_dirs_filter))
            return d_dirs_filter
    ...
    def binning(complete_dataset):
        ...
        for data in complete_dataset:
            ...
            for mz,tic in zip(mz_data,tic_data):
                ...
                loc = bisect.bisect_right(bins, mz) - 1
   ...
   #===============================================================================================================================#
    ...
    def princomp(A,numpc=0):
        ...
        for i,col in enumerate(A.T):
            if list(col).count(0) == obs:
                index.append(i)
 
        A = np.delete(A, index, axis=1)# deleting columns in which all entries are zero.# tile method?

        for n in A:
            mu = np.mean(n)
            std = np.std(n,ddof=1)
            row = [(i-mu)/std for i in n]
            data.append(row)

        B = np.vstack(data)

        C = np.cov(B.T, rowvar=0, ddof=1)
        ...
        c = [list(n) for n in projected_data.T]
        z = [0]*obs
    
        for ind in index:
            c.insert(ind,z[:])

        projected_data_full = np.array([np.array(n) for n in c]).T
        
        # think this is the projected data plus the zero rows to form the bijection between bin and mass dimension. 
        # need to fix please enter a corresponding number to open file after pca is done! switch straight to plotting data.
        return {'projected_data':projected_data_full, 'explained_variance':explained_variance, 'eigenvalues':value, 'vector':vector,'variance_value':var_val}
...
#===================================================================================================================================#
    ...
    def file_or_subdirectories_check(user_path_map, counter, current_tree, type_of_check):

        all_paths = os.walk(user_path_map[counter]).next()
        all_files = all_paths[2]
        
        if type_of_check == 'raw':  
            all_dirs = [n for n in all_paths[1] if 'raw' in n and 'pca_results' not in n]  

        elif type_of_check == 'PCA':    
            all_dirs = [n for n in all_paths[1] if 'raw' not in n and 'pca_results' not in n]
...
#===================================================================================================================================#
    ...
    def CLI():
      ...
      #  rootdir_output = print_current_dir(rootdir)#,CLI_output_type = 'initial')
        ...
        if pca_query == False:
            ...
            dynamical_output = print_current_dir(rootdir,CLI_output_type = 'initial_filter')#####
            ...
            while counter == True or end_directory == False:
                ...
                dynamical_path = [os.path.join(user_path_map[counter],all_dirs[user_input-1])]
                user_path_map.extend(dynamical_path)
                dynamical_output = print_current_dir(dynamical_path[0], CLI_output_type = 'filter')#####
            ...
            dynamical_output = print_current_dir(rootdir,CLI_output_type = 'initial_PCA')#####
            ...
            while counter == True or end_directory == False:
                ...
                dynamical_path = [os.path.join(user_path_map[counter],all_dirs[user_input-1])]
                user_path_map.extend(dynamical_path)
                dynamical_output = print_current_dir(dynamical_path[0],CLI_output_type= 'PCA')#, raw_data = True)#####
                current_tree = os.walk(dynamical_path[0]).next()
                ...
                if end_directory == True:
                    ...
                    [obs, dims] = np.shape(binned_pca)
                    pca_engage = princomp(np.array(binned_pca),numpc = obs)
                    
                    variance_value = pca_engage['variance_value']
                    projected_data = pca_engage['projected_data']
                    explained_variance = pca_engage['explained_variance']
                    eigenvalues = pca_engage['eigenvalues']
                    principal_components = pca_engage['vector']
      
                    create_directory(all_dirs[user_input-1]+'_pca_results', pca_output_name(dir_path)+'_pca_results')
                    save(projected_data,all_dirs[user_input-1]+'_projected_data', pca_output_name(dir_path)+'_pca_results')
                    save(principal_components,all_dirs[user_input-1]+'_principle_cpmts' ,pca_output_name(dir_path)+'_pca_results')
                    save(eigenvalues,all_dirs[user_input-1]+'_eigenvalues', pca_output_name(dir_path)+'_pca_results')
                    save(eigenvalues,all_dirs[user_input-1]+'_explained_variance', pca_output_name(dir_path)+'_pca_results')  
                    ...
                    if plot_query == True:
                      ...
                      print "There are",obs," principal components"
                      ...
                      first_pc_question = "Please choose a principal component for the x-axis: "
                      second_pc_question = "Please choose a principal component for the y-axis: "

                      pc_questions = [first_pc_question,second_pc_question]
                      correct_response_pc = False
                      question_results = []

                      while correct_response_pc == False:
                        try:
                          print ''
                          for q in pc_questions:
                            user_input = int(raw_input(q))
                            question_results.append(user_input)
                          print ''
                      
                          if 0 < user_input <= obs:
                            pc_axes = []

                            for n in question_results:
                              pc_individual_axis = column(principal_components, n-1)
                              pc_axes.append(pc_individual_axis)
                            
                            correct_response_pc = True
                            
                          else:
                            print 'Please enter a number between 1 and ',obs

                        except ValueError:
                          print 'Incorrect input. Please enter a number between 1 and',obs,'.'

                      #=========================================================================#

                      plot(pc_axes[0],pc_axes[1],'.')
                      print 'Data has been plotted in external window.'
                      show()
